...
public class UserService {
	...
	public Map<String, String> userComplete(@RequestParam("file") MultipartFile file, Map<String, String> param) {
		...
		this.saveGoods(param, path);
		return result;
	}

	private boolean saveGoods(Map<String, String> param, String path) {
	...
	public boolean userLogin(Map<String, String> param) {
		boolean iftrue = false;
		String sql = "select * from user_info where user_name='" + param.get("userName") + "' and user_password='"
				+ param.get("userPassword") + "'";
		List<Map<String, Object>> a = jdbc.queryForList(sql);
		if (a.size() != 0) {
			iftrue = true;
		}
